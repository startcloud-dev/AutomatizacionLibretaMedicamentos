 ...
 */
public class AgregarBodega extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (new dao.BodegaDao().agregar(dto)) {
                mensaje="Registro almacenado";
            }else{
                mensaje="Registro no se almaceno";
            }
            //Envio mensaje a la  pagina 
            request.setAttribute("mensaje", mensaje);
            request.setAttribute("lista",  new dao.BodegaDao().listar());

            request.getRequestDispatcher("Bodega/AgregarBodega.jsp").forward(request, response);
